...
public class NotificationsPresenter implements NotificationsPresenterContract,
    ...
    public NotificationsPresenter(BaseRequest.RenewTokenFailed listener, NotificationsFragmentView view,
        ...
        this.view = view;
        repository = new NotificationsRepository(this, listener);
    ...
    private void onNotificationListUpdated() {
        ...
        for (NotificationRest notificationRest : notificationsRealm) {
            ...
            switch (notificationRest.getType()) {
                ...
                case "NEW_MESSAGE":
                    Dynamizer dynamizer = groupsDb.findDynamizer(notificationRest.getIdUser());
                    Contact contact;
                    if (dynamizer == null) {
                        GetUser getUser = usersDb.findUser(notificationRest.getIdUser());
                    ...
                    if ("NEW_MESSAGE".equals(notificationRest.getType())
                        ...
                        notification.setShouldShowButton(usersDb.findUserCircle(contact.getId())
                                != null || usersDb.findCircleUser(contact.getId()) != null);
                    }
                    break;
                ...
                case "REMOVED_FROM_GROUP":
                    GroupRealm group = groupsDb.getGroupFromIdChat(notification.getIdChat());
                    Dynamizer dynamizer1 = null;
                    if (group != null) {
                        notification.setIdUser(group.getIdGroup());
                        groups.put(group.getIdGroup(), group);
                    } else {
                        dynamizer1 = groupsDb.findDynamizerFromChatId(notification.getIdChat());
                ...
                case NotificationsDb.MISSED_CALL_NOTIFICATION_TYPE:
                    GetUser getUser = usersDb.findUser(notificationRest.getIdUser());
                    users.put(getUser.getId(), createContactFromGetUser(getUser));


                    if (NotificationsDb.MISSED_CALL_NOTIFICATION_TYPE.equals(notificationRest.getType())) {
                        UsersDb usersDb = new UsersDb(MyApplication.getAppContext());
                        notification.setShouldShowButton(usersDb.findUserCircle(getUser.getId())
                                != null || usersDb.findCircleUser(getUser.getId()) != null);
                    }
                    break;
