...
public abstract class ChatRepository extends Fragment implements GetUserPhotoRequest.OnResponse, GetContentInMyGallery.OnResponse {
    ...
    public static ChatRepository newInstance(Callback listener,
                                             BaseRequest.RenewTokenFailed renewTokenListener,
                                             int userId, int chatId, boolean isGroupChat, boolean isDynamizer) {
        if (isGroupChat) return new ChatRepositoryGroup(listener, renewTokenListener, userId, chatId, isDynamizer);
        return new ChatRepositoryUser(listener, renewTokenListener, userId, chatId);
    }
    ...
    public abstract ArrayList<GetUser> getUserListData();
