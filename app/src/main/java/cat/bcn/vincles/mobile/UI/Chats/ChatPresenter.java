...
public class ChatPresenter implements ChatPresenterContract, ChatRepository.Callback, ChatAdapter.FileRequest {
    ...
    private RealmResults<NotificationRest> missedCallsRest = null;
    ...
    public ChatPresenter(BaseRequest.RenewTokenFailed listener, ChatFragmentView view,
        ...
        /*chatRepository = ChatRepository.newInstance(this, listener, idUserMe,
                Integer.parseInt(idChat), isGroupChat);*/
        isFirstTime = true;
    }
    ...
    public void loadData() {
        ArrayList<GetUser> userList = chatRepository.getUserListData();
    ...
    public void onClickSendAudio() {
      try {
        ...
        view.showBottomBar();
        if (audioRecorderFragment != null) {
            audioRecorderFragment.stopRecording(true);
        }
      } catch (Exception e) {
    ...
    public void onClickCancelMessage() {
        if (isWritingMode) {
            ...
            view.showBottomBar();
            if (audioRecorderFragment != null) {
                audioRecorderFragment.stopRecording(false);
            }
        }
    }
        ...
        for (ChatElement element : chatElements) {
            if (element instanceof ChatMessageMedia && ((ChatMessage) element).getId() == messageID) {
                ...
                for (int i = 0; i<files.size(); i++) {
                    if (files.get(i) == null || files.get(i).length() == 0){
                        files.remove(i);
    ...
    public void messagesReceived(ArrayList<ChatMessageRepositoryModel> repositoryMessageList) {
        ...
        if (repositoryMessageList.size() < 10) olderMessagesExist = false;
        ...
        if (repositoryMessageList.size() > 0 && chatElements.size() > 0) {
            ...
            if (areCalendarsSameDay(c1,c2) && chatElements.get(chatElements.size()-1).getType()
                    != ChatElement.TYPE_ALERT_DATE) {
        ...
        for (ChatMessageRepositoryModel chatMessage : repositoryMessageList) {
            if (chatMessage.getIdAdjuntContents() != null && chatMessage.getIdAdjuntContents().size() > 0) {
                ArrayList<String> paths = chatMessage.getPathsAdjuntContents();
                if (paths != null && paths.size() > 0) {
                    //request any path that is missing
                    int i = 0;
                    ArrayList<Integer> ids = new ArrayList<>();
                    for (String path : paths) {
                        if (path == null || path.length() <= 0) {
                            ids.add(chatMessage.getIdAdjuntContents().get(i));
                        }
                        i++;
                    }
                    if (isAutodownload) {
                        numberFilesDownloading = numberFilesDownloading + ids.size();
                        chatRepository.getMediaFiles(chatMessage.getId(),
                                OtherUtils.convertIntegers(ids));
                    }
                } else { //no paths, request all files
                    if (isAutodownload) {
                        numberFilesDownloading = numberFilesDownloading
                                + chatMessage.getIdAdjuntContents().size();
                        chatRepository.getMediaFiles(chatMessage.getId(),
                                OtherUtils.convertIntegers(chatMessage.getIdAdjuntContents()));
                    }
                }
            }

            /*if (anyMessageIsWatched && chatMessage.isWatched()) {
                anyMessageIsWatched = true;
            ...
            if (chatElements.size() > 0) {
                ...
                if (!areCalendarsSameDay(calendarLast, calendar)) {
                    ...
                    if (lastElement instanceof ChatMessage &&
                            ((ChatMessage) lastElement).getIdUserFrom()
                                    != chatMessage.getIdSender()) {
                        setMessageToTypeFirst(lastElement);
                    }
                }
            }

            chatElements.add(createChatElementFromRepositoryModel(chatMessage, false,
                    false));
        ...
        messagesReady = true;
        Log.d("chatorcha","presenter messagesReady");
        notifyViewIfDataReady();
    ...
    private ChatElement createMissedCallChatElement(NotificationRest missedCall) {
        ...
        return new ChatMessage(ChatElement.TYPE_ALERT_MISSED_CALL,
                missedCall.getCreationTime(),message,-1, missedCall.getIdUser(),
                "", true);
    }
    ...
    private ChatMessage createChatElementFromRepositoryModel(ChatMessageRepositoryModel chatMessage,
        ...
        if (chatMessage.getMetadataTipus().toLowerCase().contains("audio")) { //it is audio
    ...
    @Override
    public void onFileRequest(int messageId, int position) {
        numberFilesDownloading++;
        chatRepository.getMediaFile(messageId, position);
    }
}
